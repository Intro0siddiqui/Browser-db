...
fn test_ffi_lifecycle() {
    ...
    let db = zawradb_open(path.as_ptr());
    assert!(!db.is_null());

    let url = CString::new("https://example.com").unwrap();
    let title = CString::new("Example Title").unwrap();

    let res = zawradb_history_insert(db, url.as_ptr(), title.as_ptr(), 1);
    assert_eq!(res, 0);
    ...
    let title_ptr = zawradb_history_get_title(db, (url_hash & 0xFFFFFFFFFFFFFFFF) as u64, (url_hash >> 64) as u64);
    assert!(!title_ptr.is_null());

    let title_c = unsafe { CStr::from_ptr(title_ptr) };
    assert_eq!(title_c.to_str().unwrap(), "Example Title");

    zawradb_free_string(title_ptr);
    zawradb_close(db);
}
